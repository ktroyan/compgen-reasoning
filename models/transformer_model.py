...
- Defines TransformerModel, a standard Transformer model (Transformer encoder with an MLP decoder),
  implemented as a PTL LightningModule.

- Encodes the full input sequence once with a TransformerEncoder (no iterative refinement),
  extracts the grid token embeddings, and projects them to output logits via an MLPDecoder.

- Uses PTL automatic optimization with an optional linear LR warmup applied via optimizer_step.
...
class TransformerModel(ModelModule):
    ...
    def forward(self,
        ...
        """
        Forward pass: Encoder -> MLP Decoder -> Logits

        Args:
            src: input sequence of token IDs [B, S] (includes BOS, optional task tokens, grid tokens, EOS)
            tgt: target sequence of token IDs [B, S_grid + 1]; may be needed for teacher forcing in the decoder
            task_tokens: unused (task tokens are already embedded in src by the data module; kept for signature compatibility)

        Returns:
            logits: [B, S_grid, output_vocab_size]

        """
        x = self.encoder(src)   # [B, S, D]
        x = self.forward_features_for_MLP_decoder(x)    # [B, S_grid, D]
        logits = self.decoder(x)    # [B, S_grid, output_vocab_size]
