...
class TRMModel(ModelModule):
    ...
    def forward_features_for_MLP_decoder(self, x: torch.Tensor, tgt: torch.Tensor) -> torch.Tensor:
        ...
        use_tasks = self.cfg.model.get("use_task_tokens", True)
        max_task_len = self.cfg.model.get("max_task_seq_len", 0) if use_tasks else 0

        start_idx = 1 + max_task_len
        end_idx = start_idx + grid_len
    ...
    def forward(self, src: torch.Tensor, tgt: Optional[torch.Tensor] = None, task_tokens: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        # --- MLP Decoder ---
        if self.cfg.network.decoder.get("name", None) in ["mlp_decoder", "trm_decoder"]:
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Get predictions from logits: [B, S] <-- [B, S, output_vocab_size]
        preds = torch.argmax(logits, dim=-1)    # discrete tokens
