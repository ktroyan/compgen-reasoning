...
class MHSA(nn.Module):
    ...
    def __init__(self, cfg, d_model, n_heads, attn_drop_p=0., proj_drop_p=0., qkv_bias=False):
        ...
        if self.rpe_type == "1d-rope":
            self.rotary_emb = RotaryEmbedding(self.head_embed_dim, 
                                              max_seq_len, 
                                              10000
                                              )
...
class TRMEncoderLayer(nn.Module):
    def __init__(self, cfg: DictConfig, d_model: int, n_heads: int, d_ff: int):
        ...
        if ff_type == "mlp":
            ...
            self.ff_block = ConvSwiGLU(d_model, d_ff, ff_dropout_p)
...
class TRMEncoder(nn.Module):
    ...
    def latent_recursion(self, x, y, z, src_key_padding_mask):
        """
    ...
    def forward(self, src, y=None, z=None):
        ...
        Args:
            src: [B, S] input tokens
            y: [B, S, D] current answer embeddings
            z: [B, S, D] latent state

        Returns:
            y_grad: final refined answer (with gradients)
            y_next: detached answer for next outer step
            z_next: detached latent state
        """

        # Check if any input token ID is larger than what the embedding table can handle
        ...
        z = self.latent_recursion(x, y, z, src_key_padding_mask)
